...
void eu0078 :: solucion(){
	...
	while( temp_sig_1d_1[pp-1] == 0 ){
		...
		do{
			temp_sig_1d_1[temp_3]++; // Intanta sumarle la unidad mas pequena (3 en la primera iteracion)
			temp_4 = temp_2 + (temp_3+1);
			if( temp_4>pp ){  // Si es mas grande de pp, 
				temp_sig_1d_1[temp_3]--;
				temp_2 = temp_2 - (temp_3+1)*(temp_sig_1d_1[temp_3]);
				temp_sig_1d_1[temp_3] = 0;
			}
			else{
				temp_2 = temp_4;
				temp_11 = 0;
		...
		temp_sig_1d_1[0] = pp - temp_2; // Lo que falte para pp, lo guarda en unidades
		temp_1++; // Este contador es para los casos cuando por ejemplo tengamos pp, como 50+50, en ese caso las unidades son cero, pero aun asi esta es una representacion mas, pero el contador por defecto de unidades a 2ces (primer wile) no se va a activar
	}
	temp_1++; // Cuenta con el caso en que no es una suma sino simplemente el numero
	cout<<endl<<pp<<"	"<<temp_1;
	cin>>temp_15;
	if( temp_1%1000000==0 ){
		break;
